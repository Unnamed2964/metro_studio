...
- 站点重命名、线路一键新增（自动差异色）与线路属性编辑（颜色/状态/线型/环线），以及按选中线段迁移所属线
...
- 编辑交互:
  ...
  - 新增 AI 点站模式：空白点击后先落站，点击已有站点则直接复命名；统一抓取周边 OSM 语义要素（默认半径 `150m`，由 `STATION_NAMING_RADIUS_METERS` 控制；道路/地域/公共设施/建筑），调用 BLTCY API（默认 `gemini-3-flash-preview`）基于完整证据返回命名候选，并在鼠标处弹出可点击菜单应用中英文站名。
