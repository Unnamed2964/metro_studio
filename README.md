...
- 真实地图编辑器、官方风示意图、车辆 HUD 采用工作区阶段化 Tab 单屏切换（阶段一：网络编辑 / 阶段二：版式校核 / 阶段三：车载发布）
  - 站点重命名、线路一键新增（自动差异色）与线路属性编辑（颜色/状态/线型），以及按选中线段迁移所属线
...
- AI 点站模式（点击新点或已有站点后抓取周边 OSM 道路/地域/公共设施/建筑，默认半径 150m，由 `STATION_NAMING_RADIUS_METERS` 控制，并调用 BLTCY API 基于完整证据集给出命名候选，鼠标处弹出菜单选择）
- 右键站点菜单支持 AI 翻译英文站名（单站）
- 连续布线模式（从一个点开始，连续点击自动连线；绘制时按距离阈值预览变色并在鼠标旁显示米数）
- 批量站名模板重命名（`{n}` 序号占位）
- 选中多站点后可进行 AI 批量命名（支持人工筛选模式与全自动模式；人工筛选复用单站逻辑按站逐个串行预生成候选；全自动启用 strict AI 仅接受模型候选，采用“上下文并发抓取 + AI 分组批请求”，可直接应用命名并支持失败重试）
...
- 全局界面字体切换（思源黑体/苹方/微软雅黑/黑体，支持本地持久化）
- 年份选择器（工具栏顶部，默认2010年，新放置的线自动使用当前编辑年份作为 openingYear）
- 关闭/刷新页面二次确认（防误关）
...
- `src/lib/osm/nearbyStationNamingContext.js`：站点周边命名语义上下文提取（默认半径 150m；道路/地域/设施/建筑，附道路等级等元数据）。
...
- OSM 导入:
  ...
  - 勾选“包含在建/规划”时，会额外导入未挂到线路 relation 的独立地铁站点节点（例如仅有 `construction:*`/`proposed:*` 标记的车站）。
   - 环线识别：基于拓扑结构自动判断（端点数量为0且站点数≥3），环线名自动去除"起终点"后缀显示（覆盖破折号、括号等常见写法）。
- 编辑交互:
- 真实地图底图采用 OSM 标准瓦片（`tile.openstreetmap.org`，MapLibre raster source）。
- 地图支持站点单选/多选、框选、批量拖动，支持线段点击选中与高亮，支持 Alt+点击线段选中整条线路（含所有站点）。
  - 导入城市线路后自动触发镜头平滑移动动画，使用 `easeInOutCubic` 高级缓动函数（小跨度 1000ms，大跨度 1200ms），仅在边界真正改变时触发。
  - 阶段一网络编辑中的站点文字标注默认显示中英文双行（若英文为空则仅显示中文）。
  - 新增 AI 点站模式：空白点击后先落站，点击已有站点则直接复命名；统一抓取周边 OSM 语义要素（默认半径 `150m`，由 `STATION_NAMING_RADIUS_METERS` 控制；道路/地域/公共设施/建筑），调用 BLTCY API（默认 `gemini-2.5-flash`）基于完整证据返回命名候选，并在鼠标处弹出可点击菜单应用中英文站名。
  ...
  - 线段/锚点点击选中采用加宽命中区与事件抑制，避免“点中后被底图点击清空”；非选择模式下点击线段会自动切回选择模式。
...
- 工具栏采用语义分组选项卡（项目与数据 / 绘制流程 / 对象属性 / 发布导出），覆盖工程管理器、OSM 导入、编辑模式与排版控制、站点/线段/线路属性编辑与导出控制；工程与导出能力固定放在工具栏，不放入右键菜单。
  ...
  - 多站点对象属性支持 AI 批量命名流程：支持人工筛选（复用单站逻辑按站逐个串行预生成后再点选）与全自动（不筛选直接应用、可取消、可重试失败站点）；全自动使用 strict AI（无规则回退）并走“上下文并发抓取 + AI 分组批请求”链路，并显示进度。
...
- UI 主题:
  ...
  - 支持全局界面字体切换，并将用户选择持久化到 `localStorage`。
- 边界过滤:
...
- 车辆 HUD:
  - HUD 面板顶部提供线路与方向选择栏，用于阶段三独立切换视图；车载屏画布区域保持独立渲染。
  - 选择线路后自动抽取主路径并生成方向候选（开往终点/反向；环线正反向）。
  - 关键 DOM/SVG 节点已补充语义化 class（`hud-track__*` / `hud-station__*`），便于在 DevTools 中快速定位元素并做样式覆写。
  - HUD 站序自动按方向展开，渲染蓝色主线、空心站点、段间双箭头与中英站名。
  - 画布内顶部横条（Logo/线路牌/到达站面板）已移除，仅保留线路轨道与站点主体。
  - 站名字体锚点整体上移，减少与轨道视觉重叠。
  - 换乘站自动标识采用双圈样式（更粗外环 + 内圈）并保留徽标与其他线路颜色点，支持识别手动换乘关系带来的跨站换乘。
  - 环线与部分环线在线路 HUD 中按双层闭合轨道渲染（上下两层，左右端首尾连接）。
